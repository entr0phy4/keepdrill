...
number: 1
name: Chromium light + dark — empty Analytics, then populated after several mixed-language sessions
expected: |
  Empty DB shows page empty copy and no keyboard diagram. After sessions, stacked Slowest digraphs → Keyboard heatmap → Language profile; top-10 table omits thin pairs; sequential amber heatmap with on-key ms (not green/red score colors); language rows include plaintext; no shaming/guilt copy.
