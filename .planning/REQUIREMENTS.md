...
- [ ] **METR-01**: On completion, user sees net WPM for the exercise using the industry-standard formula (correct chars / 5 / minutes), matching Monkeytype's definition
- [ ] **METR-02**: On completion, user sees accuracy / error rate for the exercise (correct keypresses / total keypresses, with corrections in the denominator)
...
- [ ] **METR-04**: The metrics engine is a pure module with no I/O, re-runnable over any keystroke log; the WPM/accuracy formulas are documented in the repo and the metric schema is versioned; covered by golden-file unit tests
